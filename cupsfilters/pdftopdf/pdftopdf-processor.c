...
_cfPDFToPDFProcessingParameters_init(_cfPDFToPDFProcessingParameters *processingParams) // {{{ 
{
  ...
  processingParams->page = (_cfPDFToPDFPageRect *)malloc(sizeof(_cfPDFToPDFPageRect)); 
  _cfPDFToPDFPageRect_init(processingParams->page);
  ...
  processingParams->nup = (_cfPDFToPDFNupParameters *)malloc(sizeof(_cfPDFToPDFNupParameters));
  _cfPDFToPDFNupParameters_init(processingParams->nup);

  processingParams->input_page_ranges = (_cfPDFToPDFIntervalSet *)malloc(sizeof(_cfPDFToPDFIntervalSet));
  _cfPDFToPDFIntervalSet_init(processingParams->input_page_ranges);
  ...
  processingParams->page_ranges = (_cfPDFToPDFIntervalSet *)malloc(sizeof(_cfPDFToPDFIntervalSet));
  _cfPDFToPDFIntervalSet_init(processingParams->page_ranges);
...
void 
_cfPDFToPDFProcessingParameters_free(_cfPDFToPDFProcessingParameters *processingParams) 
{
  if (!processingParams) 
    return;
  // Free `page`
  if (processingParams->page) 
    free(processingParams->page); 
				  
  if (processingParams->nup) 
    free(processingParams->nup);
    
  if (processingParams->input_page_ranges) 
  {
    _cfPDFToPDFIntervalSet_clear(processingParams->input_page_ranges);
    free(processingParams->input_page_ranges);
  }

  if (processingParams->page_ranges) 
  { 
    _cfPDFToPDFIntervalSet_clear(processingParams->page_ranges);
    free(processingParams->page_ranges);
  }

  // Free dynamically allocated strings
  if (processingParams->page_label) 
    free(processingParams->page_label);

  // Free the structure itself
  free(processingParams);
}
...
bool 
_cfPDFToPDFProcessingParameters_even_odd_page(const _cfPDFToPDFProcessingParameters *self, 
					      int outno) // {{{
{
  if (outno % 2 == 0)
  {
		   ...
		   pdftopdf_doc_t *doc) 
{
  ...
  size_t num_page = pdfioFileGetNumPages(proc->pdf);
  
  _cfPDFToPDFPageHandle **pages = malloc((num_page) * sizeof(_cfPDFToPDFPageHandle*));
  pages = _cfPDFToPDF_PDFioProcessor_get_pages(proc, doc, &num_page);

  _cfPDFToPDFPageHandle **input_page_range_list = malloc((num_page) * sizeof(_cfPDFToPDFPageHandle*));
  size_t input_page_range_list_count = 0; // Tracks the count of valid pages

  for (size_t i = 1; i <= num_page; i++) {
    if (_cfPDFToPDFProcessingParameters_have_page(param, i)) { 
        input_page_range_list[input_page_range_list_count] = pages[i - 1];
        input_page_range_list_count++;
    }
  }

  input_page_range_list = realloc(input_page_range_list, (input_page_range_list_count) * sizeof(_cfPDFToPDFPageHandle*));
  
  const int numOrigPages = input_page_range_list_count;
  int *shuffle;
  int shuffle_size = 0; // Declare shuffle_size as an integer
  ...
  {
    shuffle = malloc(numOrigPages * sizeof(int));
  ...
  const int numPages = (shuffle_size > input_page_range_list_count) ? shuffle_size : input_page_range_list_count;
  ...
  if (param->autoprint || param->autofit)
  {
    ...
    for (int i = 0; i < input_page_range_list_count; i ++)
    {
 ...
 if (doc->logfunc) doc->logfunc(doc->logdata, CF_LOGLEVEL_DEBUG,
  ...
  if (param->pagesize_requested && (param->fillprint || param->cropfit))
  {
    for (int i = 0; i < input_page_range_list_count; i ++)
    {
      ...
      _cfPDFToPDFPageHandle_crop(page, param->page, orientation, param->orientation,
		      		 param->xpos, param->ypos,
				 !param->cropfit, param->auto_rotate,
				 doc);
    }
    if (param->fillprint)
      param->fitplot = true;
  ...
  _cfPDFToPDFNupState *nupState = (_cfPDFToPDFNupState *)malloc(sizeof(_cfPDFToPDFNupState));
  _cfPDFToPDFNupState_init(nupState, param->nup);

  _cfPDFToPDFNupPageEdit *pgedit = (_cfPDFToPDFNupPageEdit *)malloc(sizeof(_cfPDFToPDFNupPageEdit));
  ...
  for (int iA = 0; iA < numPages; iA ++)
  {
    ...
    bool newPage = _cfPDFToPDFNupState_next_page(nupState,
    ...
    if (newPage)
    {
      ...
      {
        outputno ++;
        if (_cfPDFToPDFProcessingParameters_even_odd_page(param, outputno))
        {
      ...
      curpage = _cfPDFToPDF_PDFioProcessor_new_page(proc, param->page->width, param->page->width, doc);
      outputpage++;
    }
